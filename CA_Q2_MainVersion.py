...
from skimage.morphology import disk, closing, square, dilation
...
def main():
    ...
    sigma_values = [1, 1.2, 1.3, 1.4, 1.5, 1.6]
    ...
    for i in range(len(sigma_values)):
        ...
        noise1 = addMedianFilter(noise, 2)
        ...
        dialated_img = applyClosingDilation(thresh_image)
     ...
     #   plotHistogram(threshold, noised_images, sigma_values)
        ...
        #for pltting
        sciss_overlay, props, smallest_scissors  = calculateRegionProps(labelled_img, img, 
                                                   sigma_values, i)
        small_sciss_list.append(smallest_scissors)
        smallest_scissors_overlays.append(sciss_overlay)
    ...
    plotIsolatedScissors(small_sciss_list, smallest_scissors_overlays)
    plotAllSteps(img, imag, noise, thresh_image, borders_removed, lb1, overlay, smallest_scissors )
...
def plotNoisedImages(imag, noised_images):
    ...
    for i in range(0, 3):
        for j in range(0, 2):
            ...
            ax[j, i].axis("off")
            ii += 1
    ax[0, 0].set_title("Gaussian noise $\sigma = 1$")
    ax[0, 1].set_title("Gaussian noise $\sigma = 1.2$")
    ax[0, 2].set_title("Gaussian noise $\sigma = 1.4$")
    ax[1, 0].set_title("Gaussian noise $\sigma = 1.6$")
    ax[1, 1].set_title("Gaussian noise $\sigma = 1.8$")
...
def plotRegionLabelledImages(dialated_images, sigma_values):
    ...
    for i in range(0, 2):
        for j in range(0, 3):
            ...
            ax[i, j].set_title("Dilated images {}'$\sigma = {}$'".format(ii + 1, sigma_values[ii]), fontsize = 12)
...
def plotIsolatedScissors(small_sciss_list, overlays):
    
   
          #function that uses pandas to take tha same information and put it in a csv table
    fig, ax = plt.subplots(ncols = 3, nrows=2, figsize = (10, 8))
    ii = 0
    sig = [0.02, 0.04, 0.06, 0.08, 0.1, 0.11]
    for i in range(0, 3):
        for j in range(0, 2):
            ...
            ii += 1
    ax[0, 0].set_title("smallest scissor $\sigma = 1.0$")
    ax[0, 1].set_title("smallest scissor $\sigma = 1.2$")
    ax[0, 2].set_title("smallest scissor $\sigma = 1.4$")
    ax[1, 0].set_title("smallest scissor $\sigma = 1.6$")
    ax[1, 1].set_title("smallest scissor $\sigma = 1.8$")
    ax[1, 2].set_title("smallest scissor $\sigma = 2.0$")
...
def addGaussianFilter(image, standard_dev):
    
    gaus = gaussian(image, sigma = 3, mode="reflect")
...
def calculateRegionProps(label_image, image, sigma_values, ii):
    ...
    #a few vars below. declare vars outside if statement or may gter
    #referenced before declaration error
    num_scissors = 0
    scissor_centroids = []
    scissor_areas = []
    area_of_smaller_scissors = 0
    centroid_index = 0
    centroid_of_scissor = 0
    detected = False
    ...
    for props in props:  
        ...
        #centroid
         ...
         if props.area == area_of_smaller_scissors:
        # draw rectangle around segmented coins
            over = label2rgb(label_image, image = image, bg_label=0)
    ...
    if (detected): return over, props1, smallest_scissors
    else: return smallest_scissors, props1, smallest_scissors
